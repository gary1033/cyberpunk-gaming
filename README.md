...
目前開發版本：**V0.4**。加入凱的維護責任追問、選擇即時回應、對話回看與閱讀設定、B／C 結局 CG，以及音效與休整改善。[V0.4 製作與驗證紀錄](docs/V0.4.md)。

Windows 試玩包為 **V0.4**：`NEON-MEMORIES-Windows-V0.4.zip`，解壓後執行 `NEON-MEMORIES.exe`。

第三章可先救人或先保存證據：主動對質蕭博士、從設備核對隔離報告、安排幽靈的一次協助，或停用企業資格後自行查證。原件、救援與三結局條件保留。[V0.2 製作與驗證紀錄](docs/V0.2.md)。
...
自由調查測試從新遊戲執行三入口 × 兩種目標順序 × 三結局的 18 組通關，保留原有七條支線路徑與舊存檔檢查。真人遊玩時間與實體手機不在自動通關驗證範圍。
...
- 第三章完成走廊核對後，可自由安排搜尋、隔離報告、撤離出口、原件保管鏈與主動對質。核對錯誤可重試；停用資格不抹除使用紀錄，幽靈協助一經確認不能重新分派。
- 請在對話與轉場結束後存檔。
...
- 核對所有來源指紋與路徑：`python tools/audit_assets.py`。V0.4 已補齊四個短音效缺口，另加入證據、掃描與交接提示；來源與 CC0 授權見 `assets/audio/sfx/LICENSE.md`。
