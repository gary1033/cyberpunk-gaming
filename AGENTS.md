...
- 每次劇情、台詞、證據、分支或結局更動前，另讀 `C:\Users\gary1\OneDrive\桌面\obsidian\project開發\NEON MEMORIES\NEON MEMORIES 劇情關係與改動檢查.md`。找到受影響的事件與人物，核對前置來源、角色當時已知資訊、選項實際後果及後續結局；前因、後果至少各查一層。純改口吻也適用。
- 修改後同步該關係筆記與版本紀錄，留下本次節點、修正／新增／刪減理由、測試結果與未驗證範圍。筆記與程式不一致時先標記差異，不能把推測或規劃寫成已發生事件；圖譜是人工檢查索引，不取代實際分支測試。
...
### 版本命名規則
- 目前開發版本為 V0.4，project.godot 的 config/version 為 0.4；已匯出的試玩包為 V0.4。
- 後續交付依序使用版本號（例如 V0.5）；版本名稱、試玩包與匯出資料夾不得以日期或修改內容命名。
- Windows 交付格式：NEON-MEMORIES-Windows-V0.4.zip；資料夾同名，程式入口保持 NEON-MEMORIES.exe。
- 日期、修改內容及驗證結果記錄在更新紀錄中。每次交付同步專案版本設定、匯出路徑、包內說明與 Obsidian。
